	...
	// single instance of this class to be used by the all consumers
		...
		public void insert(Product entity) {
			try (Connection c = createConnection()) {
				PreparedStatement pstmt = c.prepareStatement("insert into product(title, price, description) values(?,?,?)");
		...
		public void update(Product entity) {
			try (Connection c = createConnection()) {
				PreparedStatement pstmt = c.prepareStatement("update product set title=?, price=?, description=? where id=?");
				pstmt.setString(1, entity.getTitle());
				pstmt.setInt(2, entity.getPrice());
				pstmt.setString(3, entity.getDescription());
				pstmt.executeUpdate();
